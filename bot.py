...
async def cmd_start(message: Message, state: FSMContext):
    ...
    welcome = (
        "🚀 <b>Добро пожаловать в Synergy Hub Junior!</b>\n\n"
        "👨‍💻 Обучаем детей <b>8–15 лет</b> цифровым навыкам —\n"
        "через игру, практику и создание собственных проектов.\n\n"
        "✅ Программирование\n"
        "✅ Дизайн и графика\n"
        "✅ Робототехника\n\n"
        "📍 <b>Ташкент</b>\n\n"
        "Запишитесь на <b>бесплатный пробный урок</b> прямо сейчас 👇"
    )
    keyboard = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="🎓 Записаться на пробный урок")]],
        resize_keyboard=True,
    )
...
async def got_parent_name(message: Message, state: FSMContext):
    ...
    if text == "🎓 Записаться на пробный урок":
        await message.answer("Как вас зовут? (имя родителя)", reply_markup=ReplyKeyboardRemove())
        return
