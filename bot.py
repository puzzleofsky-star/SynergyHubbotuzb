...
async def cmd_start(message: Message, state: FSMContext):
    ...
    welcome = (
        "🚀 <b>Добро пожаловать в Synergy Hub Junior</b> — международный детский IT-центр в Ташкенте!\n\n"
        ...
        "☀️ <b>Летние IT-каникулы</b> — 10 дней, реальный проект\n"
        "👾 8–11 лет — «Код выживания»\n"
        "🤖 12–15 лет — «Нейросети и чат-боты»\n\n"
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="☀️ Летние IT-каникулы")],
            [KeyboardButton(text="💬 Получить консультацию менеджера")],
        ],
        resize_keyboard=True,
    )
...
async def got_parent_name(message: Message, state: FSMContext):
    ...
    if text == "🎓 Записаться на пробный урок":
        ...
        return
    if text == "☀️ Летние IT-каникулы":
        camp_text = (
            "☀️ <b>Летние IT-каникулы в Synergy Hub Junior</b>\n\n"
            "Ваш ребёнок много времени в телефоне?\n"
            "Пусть сам создаёт технологии, а не просто потребляет.\n\n"
            "За 10 дней — реальный проект, который можно показать друзьям.\n\n"
            "👾 <b>8–11 лет — «Код выживания»</b>\n"
            "• Программирование в Scratch\n"
            "• Графика и монтаж видео\n"
            "• Защита данных и пароли\n"
            "• Собственная игра с нуля\n\n"
            "🤖 <b>12–15 лет — «Нейросети и чат-боты»</b>\n"
            "• ИИ-ассистенты для учёбы и экзаменов\n"
            "• Личный ИИ-репетитор — бесплатно и навсегда\n"
            "• Презентации в Gamma\n"
            "• Чат-бот с нуля\n\n"
            "🏆 В финале — защита проекта и именной сертификат.\n"
            "Плюс квесты, кино, прогулки и мастер-классы."
        )
        camp_keyboard = ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="📝 Записаться на пробный день")],
                [KeyboardButton(text="💬 Получить консультацию менеджера")],
            ],
            resize_keyboard=True,
        )
        await message.answer(camp_text, parse_mode="HTML", reply_markup=camp_keyboard)
        await state.set_state(CampChoice.choosing)
        return
    if text == "💬 Получить консультацию менеджера":
        await state.update_data(type="Консультация")
        await message.answer(
...
# ─── Выбор после каникул ─────────────────────────────────────
async def camp_choice(message: Message, state: FSMContext):
    text = message.text.strip()
    if text == "📝 Записаться на пробный день":
        age_keyboard = ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="👾 8–11 лет — «Код выживания»")],
                [KeyboardButton(text="🤖 12–15 лет — «Нейросети и чат-боты»")],
            ],
            resize_keyboard=True,
        )
        await message.answer(
        ...
        await state.set_state(CampChoice.age)
    elif text == "💬 Получить консультацию менеджера":
        await state.update_data(type="Консультация")
        await message.answer(
...
async def camp_age(message: Message, state: FSMContext):
    text = message.text.strip()
    if text == "👾 8–11 лет — «Код выживания»":
        program = "Код выживания (8–11 лет)"
    elif text == "🤖 12–15 лет — «Нейросети и чат-боты»":
        program = "Нейросети и чат-боты (12–15 лет)"
    else:
        await message.answer("Пожалуйста, выберите один из вариантов 👇")
        return
    await state.update_data(type=f"Пробный день — {program}")
    await message.answer(
        f"Отличный выбор! 🎉\n\nКак вас зовут?",
        reply_markup=ReplyKeyboardRemove()
    )
...
async def consult_phone(message: Message, state: FSMContext, bot: Bot):
    ...
    lead_type = data.get("type", "Консультация")
...
async def main():
    ...
    dp.message.register(camp_choice,   CampChoice.choosing)
    dp.message.register(camp_age,      CampChoice.age)
